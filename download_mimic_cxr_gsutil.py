#!/usr/bin/env python3
"""
Download MIMIC-CXR dataset using gsutil (Google Cloud Storage).
Requires gcloud authentication and a GCP project ID for requester-pays bucket.
...
Prerequisites:
  ...
  4. Link your Google account to PhysioNet for bucket access
"""
import subprocess
import os
import sys
...
BUCKET = "gs://mimic-cxr-jpg-2.1.0.physionet.org"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download MIMIC-CXR dataset from GCS with optional subset filtering"
    )
    parser.add_argument(
        "--project-id",
        type=str,
        required=True,
        help="GCP project ID for requester-pays bucket billing",
    )
    parser.add_argument(
        "--subset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of parallel download threads for xargs (default: 4)",
    )
    parser.add_argument(
        "--skip-existing",
        type=bool,
    ...
    try:
        ...
        if FILE_LIST_CACHE.exists():
            ...
            # Download the file list via gsutil
            cmd_list = [
                "gsutil", "-u", PROJECT_ID, "cp",
                f"{BUCKET}/IMAGE_FILENAMES", list_file,
            ]
        ...
        # Step 2: Download files in parallel using gsutil
        # Each file appends to manifest ONLY on successful gsutil cp (exit code 0)
        # gsutil -q suppresses progress output for cleaner logs
        print(f"Downloading files (parallel with {PARALLEL_JOBS} jobs)...\n")
        cmd_download = (
            f"cat {list_file} | xargs -P {PARALLEL_JOBS} -I {{}} sh -c '"
            f"mkdir -p {OUTPUT_DIR}/$(dirname {{}}) && "
            f"gsutil -q -u {PROJECT_ID} cp "
            f"{BUCKET}/{{}} "
            f"{OUTPUT_DIR}/{{}} 2>/dev/null "
